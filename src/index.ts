...
app.get('/types/status', async (_req: Request, res: Response) => {
  try {
...
app.post('/admin/types/sync', authMiddleware, adminMiddleware, async (_req: Request, res: Response) => {
  try {
    ...
    console.error('[Admin] Types sync failed:', getErrorMessage(error));
    res.status(500).json({
      status: 'error',
