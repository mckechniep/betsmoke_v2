...
router.get('/search/:query', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Team search error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/h2h/:team1Id/:team2Id', async (req: Request, res: Response) => {
  try {
    ...
    if (isInvalidNumber(team1Id) || isInvalidNumber(team2Id)) {
      return res.status(400).json({
    ...
    console.error('Head-to-head error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/stats/seasons/:seasonId', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get team stats by season error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/stats', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get team stats error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/squad', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get team squad error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/squad/seasons/:seasonId', async (req: Request, res: Response) => {
  try {
    const { id: teamId, seasonId } = req.params;
    ...
    console.error('Get team squad by season error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/fullsquad/seasons/:seasonId', async (req: Request, res: Response) => {
  try {
    const { id: teamId, seasonId } = req.params;
    ...
    const playersWithStats = squadMembers.map((member: any) => {
        ...
        details.forEach(detail => {
          ...
          const getValue = (val: unknown) => {
            ...
            if (val && typeof val === 'object') {
              const valueObj = val as { total?: number; count?: number };
              return valueObj.total || valueObj.count || 0;
            }
            return 0;
          };
    ...
    playersWithStats.sort((a: any, b: any) => {
    ...
    console.error('Get team full squad error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/topstats/seasons/:seasonId', async (req: Request, res: Response) => {
  try {
    const { id: teamId, seasonId } = req.params;
    ...
    console.error('Get team top stats error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/transfers', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get team transfers error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/seasons', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get team seasons error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/schedule', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get team schedule error:', getErrorMessage(error));
    res.status(500).json({ 
 ...
 */
router.get('/coaches/search/:query', async (req: Request, res: Response) => {
  try {
    const searchQuery = req.params.query;
    ...
    console.error('Coach search error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/coaches/:id', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get coach error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.get('/:id/corners/seasons/:seasonId', async (req: Request, res: Response) => {
  try {
    const { id: teamId, seasonId } = req.params;
    ...
    let seasonData = cache.get(seasonCacheKey) as any;
    ...
    const fixtures = allFixtures.filter((f: any) => f.season_id === parseInt(seasonId, 10));
    ...
    let awayCorners = 0;
    let awayGames = 0;
    const skippedFixtures: Array<{
      fixtureId: number;
      teamLocation?: string;
      state?: string;
      participantMeta?: unknown;
    }> = [];
    ...
    console.error('Get team corner averages error:', getErrorMessage(error));
    res.status(500).json({ 
...
router.delete('/:id/corners/seasons/:seasonId/cache', async (req: Request, res: Response) => {
  try {
    const { id: teamId, seasonId } = req.params;
    ...
    console.error('Clear cache error:', getErrorMessage(error));
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get team error:', getErrorMessage(error));
    res.status(500).json({ 
