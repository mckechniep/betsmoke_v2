...
router.patch('/email', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    console.error('Change email error:', getErrorMessage(error));
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.patch('/security-question', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    console.error('Update security question error:', getErrorMessage(error));
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/get-security-question', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Get security question error:', getErrorMessage(error));
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/verify-security-answer', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Verify security answer error:', getErrorMessage(error));
    res.status(500).json({ error: 'Server error' });
  }
});
