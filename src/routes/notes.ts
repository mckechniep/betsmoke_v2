...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const { title, content, links } = req.body as {
      title?: string;
      content?: string;
      links?: NoteLinkInput[];
    };
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const { contextType, contextId } = req.query as {
      contextType?: string;
      contextId?: string;
    };
    ...
    const whereClause: {
      userId: string;
      links?: { some: { contextType?: string; contextId?: string } };
    } = { userId };
    ...
    console.error('Error fetching notes:', getErrorMessage(error));
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Error fetching note:', getErrorMessage(error));
    res.status(500).json({ error: 'Server error' });
  }
});
