...
let typesById: Map<number, SportsMonksType> | null = null;        // Map: id -> type object
let typesByCode: Map<string, SportsMonksType> | null = null;      // Map: code -> type object
let typesByModelType: Map<string, SportsMonksType[]> | null = null; // Map: modelType -> array of types
let cacheLoadedAt: Date | null = null;    // When the cache was last loaded
 ...
 */
const loadTypesCache = async (): Promise<number> => {
 ...
 * @returns {Promise<number>} - Number of types loaded
 */
const refreshCache = async (): Promise<number> => {
  typesById = null;
  typesByCode = null;
 ...
 */
const getTypeById = async (id: number): Promise<SportsMonksType | null> => {
 ...
 * // Returns: "Corners"
 */
const getTypeName = async (id: number): Promise<string> => {
 ...
 */
const getTypeByCode = async (code: string): Promise<SportsMonksType | null> => {
  await ensureCacheLoaded();
 ...
 */
const getTypesByModelType = async (modelType: string): Promise<SportsMonksType[]> => {
  await ensureCacheLoaded();
 ...
 */
const getTypesByIds = async (ids: number[]): Promise<Map<number, SportsMonksType>> => {
  await ensureCacheLoaded();
  const result = new Map<number, SportsMonksType>();
  for (const id of ids) {
...
const syncTypesFromAPI = async () => {
  ...
  const allIds = new Set(apiTypes.map((t: any) => t.id));
 ...
 */
const enrichStatWithType = async (stat: Record<string, any>) => {
  if (stat?.type_id) {
 ...
 */
const enrichStatsWithTypes = async (stats: Array<Record<string, any>>) => {
 ...
 */
const enrichEventsWithTypes = async (events: Array<Record<string, any>>) => {
 ...
 */
const enrichFixtureWithTypes = async (fixture: Record<string, any>) => {
