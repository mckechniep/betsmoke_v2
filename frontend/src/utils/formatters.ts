...
export function parseUTCDateTime(dateString?: string | null): Date | null {
...
export function formatTime(dateString?: string | null, timezone = 'America/New_York') {
...
// dateFormat: 'US' = "Friday, January 25, 2026"
// dateFormat: 'EU' = "Friday, 25 January 2026"
export function formatDateOnly(dateString?: string | null, dateFormat: 'US' | 'EU' = 'US') {
...
export function formatShortDateOnly(dateString?: string | null, dateFormat: 'US' | 'EU' = 'US') {
  const date = parseDateString(dateString);
  if (!date) return '';
...
export function getTimezoneDateString(dateString?: string | null, timezone = 'America/New_York') {
  const date = parseUTCDateTime(dateString);
  if (!date) return '';
...
// unit: 'CELSIUS' = "22°C"
export function formatTemperature(celsiusValue: number | null | undefined, unit: 'FAHRENHEIT' | 'CELSIUS' = 'FAHRENHEIT') {
...
export function getOddsValue(odd: OddsValue, format: 'AMERICAN' | 'DECIMAL' | 'FRACTIONAL' = 'AMERICAN') {
  if (!odd) return null;
...
export function formatOdds(odd: OddsValue, format: 'AMERICAN' | 'DECIMAL' | 'FRACTIONAL' = 'AMERICAN') {
...
export function formatAmericanOdds(americanOdd: string | number | null | undefined) {
...
export function getOddsFormatLabel(format: 'AMERICAN' | 'DECIMAL' | 'FRACTIONAL') {
  switch (format) {
    case 'DECIMAL':
      return 'Decimal';
    case 'FRACTIONAL':
